            ...
            for (var i = 0, len = matches.length; i < len; i++) {
                if (matches[i].url.indexOf('data:image') === -1) { //if find -> image already decoded
                    ...
                    if (fs.existsSync(filepath)) {
                        ...
                        } else if (matches[i].url.indexOf('svg') === -1 && matches[i].url.indexOf('png') === -1) {
                            if (opts.debug) gutil.log('gulp-inline-base64: Skipping non image:' + gutil.colors.gray(' (' + filepath + ')'));
                        }
